...
def get_api ():
    ...
    host = "https://openshift.default.svc.cluster.local"
    ssl_ca_cert = "/var/run/secrets/kubernetes.io/serviceaccount/ca.crt"
    api = rest_api.Api(host, api_key, api_key_prefix, ssl_ca_cert, '/.vbmc/api.log')
...
class VirtualBMC(bmc.Bmc):
    ...
    def get_power_state(self):
        ...
        if CONF['default']['kubevirt'] != 'true':
            ...
            api = get_api()
            json = api.vm_status(self.namespace,self.name)
            if 'ready' not in json['status']:
                return POWEROFF
            else:
                if json['status']['ready'] is True:
                    return POWERON
    ...
    def power_off(self):
        ...
        if CONF['default']['kubevirt'] != 'true':
            ...
            api = get_api()
            api.vm_action(self.namespace,self.name,'stop')
    ...
    def power_on(self):
        ...
        if CONF['default']['kubevirt'] != 'true':
            ...
            api = get_api()
            api.vm_action(self.namespace,self.name,'start')
    ...
    def power_reset(self):
        ...
        if CONF['default']['kubevirt'] != 'true':
            ...
            api = get_api()
            api.vm_action(self.namespace,self.name,'restart')
            #LOG.info('reset option not supported for domain %(domain)s', {'domain': self.domain_name} )
            #return IPMI_COMMAND_NODE_BUSY
